...
| [decomposition-output-budget.md](decomposition-output-budget.md) | Pre-decomposition planner (`task-planner` agent + `ai.task.split` / `ai.task.decompose` topics) — bounds reasoning-model output budget by emitting one small decision per `work.task.assigned`. Parent/child Task model with `TASK_STATUS_SPLIT` roll-up |
